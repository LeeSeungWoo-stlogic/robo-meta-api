...
def main() -> int:
    ...
    if r.status_code != 200:
        ...
        threshold = obj.get("threshold_used") or {}
        if "join_groups_mode" not in threshold:
            failures.append("data_decision: threshold_used.join_groups_mode missing")
        for jg in obj.get("join_groups") or []:
            for b in jg.get("bridges") or []:
                via = b.get("via")
                if via not in ("fk", "ontology", "convention", "embedding", "term"):
                    failures.append(f"data_decision: invalid bridge via={via}")
                if via == "convention":
                    path = b.get("path") or []
                    if b.get("confidence", 1.0) > 0.6:
                        failures.append("data_decision: convention confidence too high")
                    if not any(str(p).startswith("shared_column:") for p in path):
                        failures.append("data_decision: convention missing shared_column path")
                    if not any(str(p).startswith("dtype:") for p in path):
                        failures.append("data_decision: convention missing dtype path")
                    if any(p == "cast_recommended:true" for p in path):
                        if b.get("confidence", 1.0) > 0.5:
                            failures.append(
                                "data_decision: cast_recommended but confidence > 0.5"
                            )
    jg_mode = (obj.get("threshold_used") or {}).get("join_groups_mode", "empty")
    jg_count = len(obj.get("join_groups") or [])
    print(f"[2/8] /data_decision -> {r.status_code} target={obj.get('target')} cands={len(cands)} join_groups={jg_count} mode={jg_mode}")
